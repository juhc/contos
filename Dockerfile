...
RUN mkdir -p usr/bin && \
    wget -qO- https://download.docker.com/linux/static/stable/x86_64/docker-${DOCKER_VERSIONVERSION}.tgz | tar xJ -C usr/bin
...
RUN mkdir -p usr/bin && \
    wget -qO usr/bin/dumb-init 1/v${DINIT_VERSION}/dumb-init_${DINIT_VERSION}_x86_64 && \
